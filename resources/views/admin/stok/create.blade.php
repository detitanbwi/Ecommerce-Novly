...
<form action="{{ route('admin.stok.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-10">
    @csrf
    ...
    <div class="lg:col-span-2 space-y-8">
        <div class="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-6">
            <div>
                ...
                <input type="text" name="title" required value="{{ old('title') }}" placeholder="Contoh: Luxury Designer Chicken Coop" class="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all font-semibold text-slate-900">
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Deskripsi Produk</label>
                <textarea name="description" required rows="6" placeholder="Jelaskan fitur, bahan, dan detail produk lainnya..." class="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all font-medium text-slate-700 leading-relaxed">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="price" required value="{{ old('price') }}" placeholder="3500000" class="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all font-semibold text-slate-900">
                </div>
            </div>
        </div>
        ...
        <div class="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="url" name="external_links[shopee]" value="{{ old('external_links.shopee') }}" placeholder="https://shopee.co.id/..." class="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all font-medium text-slate-700">
                </div>
                <div>
    ...
    <div class="space-y-8">
        <div class="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-6">
            <div>
                <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Foto Produk (Max 3)</label>
                <div class="grid grid-cols-3 gap-4" id="image-slots">
                    @for($i = 0; $i < 3; $i++)
                        <div class="relative aspect-square group">
                            <!-- Slot Empty -->
                            <div id="slot-empty-{{ $i }}" onclick="document.getElementById('input-{{ $i }}').click()" class="w-full h-full border-2 border-dashed border-slate-200 rounded-3xl flex flex-col items-center justify-center text-slate-400 hover:border-indigo-400 hover:text-indigo-500 transition-all cursor-pointer bg-slate-50/50">
                            ...
                            <!-- Slot Filled -->
                            <div id="slot-filled-{{ $i }}" class="hidden w-full h-full rounded-3xl overflow-hidden border border-slate-200 bg-slate-100 relative">
                                <img id="preview-{{ $i }}" class="w-full h-full object-cover">
                ...
                <p class="mt-4 text-[11px] text-slate-400 leading-tight italic">Gambar akan otomatis dioptimasi agar ringan saat diakses oleh pembeli.</p>
            </div>
            ...
            <div class="pt-6 border-t border-slate-50">
                <label class="flex items-center gap-3 cursor-pointer group">
                    ...
                    <input type="checkbox" name="is_active" value="1" checked class="w-5 h-5 rounded border-slate-200 text-indigo-600 focus:ring-indigo-500">
