...
@section('content')
    <div class="max-w-6xl mx-auto">
        <!-- Breadcrumbs -->
        <nav class="flex mb-8 text-sm font-medium text-emerald-800/60" aria-label="Breadcrumb">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-16">
                    ...
                    <div class="flex gap-4">
                        @foreach($product->images as $image)
                            <button onclick="document.getElementById('main-image').src='{{ asset($image) }}'"
                                class="w-24 h-24 rounded-2xl overflow-hidden border-2 border-transparent focus:border-emerald-600 transition-all">
            ...
            <div class="flex flex-col">
                <div class="mb-4">
                    <span
                        class="inline-block px-4 py-1.5 bg-emerald-50 text-emerald-700 text-[11px] font-bold rounded-full uppercase tracking-widest">{{ $product->category->name }}</span>
                </div>

                <h1 class="text-4xl lg:text-5xl font-bold text-slate-900 tracking-tight mb-6">{{ $product->title }}</h1>

                <div class="flex items-center mb-8">
                    <span class="text-3xl lg:text-4xl font-bold text-emerald-600 tracking-tighter">Rp
                ...
                <div class="space-y-4 mt-auto">
                    ...
                    <a href="{{ $waLink }}" target="_blank"
                        class="flex items-center justify-center gap-3 w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-5 rounded-2xl transition-all shadow-lg shadow-emerald-100">
                            ...
                            @foreach($product->external_links as $platform => $url)
                                @if($url)
                                    <a href="{{ $url }}" target="_blank"
                                        class="flex items-center justify-center gap-2 bg-white border border-green-200 hover:border-emerald-600 hover:text-emerald-600 text-slate-700 font-bold py-4 rounded-2xl transition-all">
                                        {{ ucfirst($platform) }}
                                    </a>
                                @endif
                            @endforeach
